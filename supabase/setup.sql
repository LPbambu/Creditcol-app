-- CREDITCOL Database Setup Script
...
CREATE INDEX IF NOT EXISTS idx_contacts_phone ON public.contacts(user_id, phone);

-- ============================================
-- 3. EXCEL UPLOADS TABLE
-- ============================================
...
CREATE TABLE IF NOT EXISTS public.campaigns (
  ...
  name TEXT NOT NULL,
  description TEXT,
  send_type TEXT NOT NULL CHECK (send_type IN ('immediate', 'scheduled', 'manual')),
...
CREATE TABLE IF NOT EXISTS public.messages (
  ...
  content TEXT NOT NULL,
  phone TEXT NOT NULL,
  status TEXT DEFAULT 'pending' CHECK (status IN ('pending', 'queued', 'sending', 'sent', 'delivered', 'read', 'failed')),
  provider TEXT,
...
CREATE TABLE IF NOT EXISTS public.system_logs (
  ...
  action_category TEXT NOT NULL CHECK (action_category IN ('auth', 'upload', 'contact', 'campaign', 'message', 'system')),
...
-- ============================================
-- 8. WHATSAPP CONFIG TABLE
-- ============================================
CREATE TABLE IF NOT EXISTS public.whatsapp_config (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  user_id UUID NOT NULL REFERENCES public.profiles(id) ON DELETE CASCADE,
  provider TEXT NOT NULL CHECK (provider IN ('twilio', 'whatsapp-business', 'other')),
  account_sid TEXT,
  auth_token TEXT,
...
CREATE POLICY "Users can view own config" ON public.whatsapp_config
  FOR SELECT USING (auth.uid() = user_id);

CREATE POLICY "Users can insert own config" ON public.whatsapp_config
  FOR INSERT WITH CHECK (auth.uid() = user_id);

CREATE POLICY "Users can update own config" ON public.whatsapp_config
  FOR UPDATE USING (auth.uid() = user_id);
...
-- Apply to all tables
CREATE TRIGGER trigger_updated_at BEFORE UPDATE ON public.profiles
  FOR EACH ROW EXECUTE FUNCTION update_updated_at();

CREATE TRIGGER trigger_updated_at BEFORE UPDATE ON public.contacts
  FOR EACH ROW EXECUTE FUNCTION update_updated_at();

CREATE TRIGGER trigger_updated_at BEFORE UPDATE ON public.excel_uploads
  FOR EACH ROW EXECUTE FUNCTION update_updated_at();

CREATE TRIGGER trigger_updated_at BEFORE UPDATE ON public.message_templates
  FOR EACH ROW EXECUTE FUNCTION update_updated_at();

CREATE TRIGGER trigger_updated_at BEFORE UPDATE ON public.campaigns
  FOR EACH ROW EXECUTE FUNCTION update_updated_at();

CREATE TRIGGER trigger_updated_at BEFORE UPDATE ON public.messages
  FOR EACH ROW EXECUTE FUNCTION update_updated_at();

CREATE TRIGGER trigger_updated_at BEFORE UPDATE ON public.whatsapp_config
  FOR EACH ROW EXECUTE FUNCTION update_updated_at();

-- ============================================
-- 10. STORAGE BUCKET FOR UPLOADS
-- ============================================
...
SELECT 'CREDITCOL database setup complete!' AS message;
